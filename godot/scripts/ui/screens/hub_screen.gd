...
func build() -> void:
	...
	col.add_child(UIKit.header("Festival Hub", "", "Port Hercule  ·  Monaco & Riviera Open World"))
	var sp := Control.new()
	sp.custom_minimum_size = Vector2(0, 10)
	col.add_child(sp)
	var drive := UIRow.new("FREE ROAM", "RIVIERA ▶")
	drive.on_accept = func(): festival.drive()
	col.add_child(drive)
	_events_row = UIRow.new("EVENTS & CHAMPIONSHIPS")
	...
	var dealer := UIRow.new("AUTO SHOW & DEALERSHIP", "%d cars" % CarData.keys().size())
	dealer.on_accept = func(): festival.push(DealerScreen.new())
	col.add_child(dealer)
	_omikuji_row = UIRow.new("SUPER WHEELSPIN")
...
func refresh() -> void:
	...
	_omikuji_row.set_value("%d spin%s" % [draws, "" if draws == 1 else "s"], UIKit.AMBER if draws > 0 else UIKit.DIM)
	var done := 0
